...
//     }
     ...
     * @param resource_amounts Array of resource amounts to be consumed, one per resource type
     * @return True if there were enough resources (including amount_needed = 0); false if not enough, in which case none are consumed.
     * 
     */
     ...
     * @param amount_needed Amount to be consumed
     * @return Amount actually consumed (less than amount_needed, if pod doesn't have enough)
     * 
     */
     ...
     * 
     * @param num_planets Number of planets on which to create units (starting from planet 0).
     * @param num_stacks Number of stacks to create per planet. Will be paced starting from hex [0,0], land hexes only.
     * @param num_factions You'll get a mix of this number of factions on each planet (starting from faction 0). 1 = all Li Halan.
     * @param num_units Number of units to put in each stack. You'll get a variety of types.
     * @param first_type Each stack will contain one unit of each type starting at this point in UNIT.DAT (e.g. 45 = Noble).
     */
        ...
        for (int p_idx = 0; p_idx < Math.min(num_planets, planets.size()); p_idx++) {
            ...
            for (int x = 0; x < C.PLANET_MAP_WIDTH; x++) {
                for (int y = 0; y < C.PLANET_MAP_COLUMNS - 1; y++) {
                    if (stack_count >= num_stacks) break;
                    
                    // Do one hex
                    
                    Hex hex = planet.planet_grid.getHex(x, y);    // Get hex
                    List<Unit> stack = hex.getStack();    // Get stack
                    if (!stack.isEmpty()) continue;    // Skip hex if it contains units already
                    if (hex.getStructure() != null) continue;    // Skip hex if it contains a city/structure
                    if (hex.getTerrain()[C.OCEAN]) continue;    // Skip ocean hexes
                    ...
                    for (int unit_count = 0; unit_count < Math.min (num_units, 20); unit_count++) {
                        ...
                        if (unit_type == C.MoveType.NAVAL.ordinal()) continue;    // Skip Naval types
                        
                        Unit unit = game.createUnitInHex(p_idx, x, y, faction, unit_type, 0, 0, 0);

                        unit_type++;
                        if (unit_type > 91) unit_type = 0;    // Cycle back to start of the unit type list 
                    }
                    stack_count++;
                    faction++;
                    if (faction == num_factions) faction = 0;
                }
            }
        }
    }
